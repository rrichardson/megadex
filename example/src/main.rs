...
impl Veggie {
    pub fn find_by_flavor(
        ...
        md.get_by_field("flavor", field.as_bytes())
    }
    pub fn id_by_flavor<'s>(
        md: &MegadexDb<Veggie>,
        key: &String,
    ) -> Result<Vec<&'s [u8]>, MegadexDbError> {
        let envlock = md.get_env().read()?;
        let reader = envlock.read_multi()?;
        let mditer = md.get_ids_by_field(&reader, "flavor", key.as_bytes())?;
        if let Some(iter) = mditer {
            Ok(iter.map(|(k, v)| {
                let Value::Blob(i) = v.unwrap().unwrap();
                i
            }).collect::<Vec<&[u8]>>())
        } else {
            Ok(Vec::new())
    ...
    pub fn find_by_leaves(
        ...
        md.get_by_field("leaves", field.as_bytes())
    }
    pub fn id_by_leaves(
        md: &MegadexDb<Veggie>,
        key: &String,
    ) -> Result<Vec<&[u8]>, MegadexDbError> {
        let envlock = md.get_env().read()?;
        let reader = envlock.read_multi()?;
        let mditer = md.get_ids_by_field(&reader, "leaves", key.as_bytes())?;
    ...
    pub fn save(&self, md: &MegadexDb<Veggie>) -> Result<(), MegadexDbError> {
        md.put(
            ...
            self,
            [
                ("flavor", self.flavor.as_bytes()),
                ("leaves", self.leaves.as_bytes()),
            ],
        )
    }
